...
export async function getServices(): Promise<Service[]> {
  ...
  const { data, error } = await getSupabase()!.from('services').select('*').eq('is_active', true).order('base_price');
...
export async function getAllServices(): Promise<Service[]> {
  ...
  const { data, error } = await getSupabase()!.from('services').select('*').order('name');
  if (error) throw error;
  return data ?? [];
}
...
export async function upsertService(service: Partial<Service> & Pick<Service, 'name' | 'base_price'>) {
  ...
  const { data, error } = await getSupabase()!.from('services').upsert(payload).select().single();
  if (error) throw error;
  return data;
}
...
export async function getAddresses(userId: string): Promise<Address[]> {
  ...
  const { data, error } = await getSupabase()!.from('addresses').select('*').eq('user_id', userId);
  if (error) throw error;
  return data ?? [];
}
...
export async function saveAddress(address: Omit<Address, 'id'> & { id?: string }) {
  ...
  const { data, error } = await getSupabase()!.from('addresses').upsert(payload).select().single();
  if (error) throw error;
  return data;
}
...
export async function deleteAddress(id: string, userId: string) {
  ...
  const { error } = await getSupabase()!.from('addresses').delete().eq('id', id).eq('user_id', userId);
  if (error) throw error;
}
...
export async function createOrder(input: {
  ...
  const servicePrices = await Promise.all(
    input.items.map(async (item) => {
      const { data } = await getSupabase()!.from('services').select('base_price').eq('id', item.serviceId).single();
  ...
  await getSupabase()!.from('order_status_events').insert({
    order_id: order.id,
    status: 'confirmed',
...
export async function getCustomers(): Promise<(Profile & { orderCount: number })[]> {
  ...
  const { data: profiles, error } = await getSupabase()!.from('profiles').select('*').eq('role', 'customer');
  if (error) throw error;
  const { data: orders } = await getSupabase()!.from('orders').select('customer_id');
...
export async function getDashboardStats(): Promise<DashboardStats> {
  ...
  const { data: orders, error } = await getSupabase()!.from('orders').select('*');
  if (error) throw error;
  const list = orders ?? [];
  return {
